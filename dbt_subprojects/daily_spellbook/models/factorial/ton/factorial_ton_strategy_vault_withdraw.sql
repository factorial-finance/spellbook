...
trace_opcodes AS (
    SELECT 
        trace_id,
        CASE 
            WHEN COUNT(CASE WHEN opcode = 2078119902 THEN 1 END) > 0 THEN 'withdraw'
            ELSE 'unknown'
        END as transaction_type
    FROM {{ source('ton', 'messages') }} M
    WHERE M.trace_id IN (SELECT trace_id FROM parsed_boc) AND M.destination IN (SELECT vault_address FROM parsed_boc)
...
WHERE p.result.owner_address != 'addr_none' AND t.transaction_type = 'withdraw'